...
Browser::Browser(sf::Window& window) : window_(window) {
	...
	config.device_scale_hint = 1.f;//window_.scale();
	config.use_distance_field_fonts = 1.f != 1.0f;//window_.scale() != 1.0f; // Only use SDF fonts for high-DPI
...
void Browser::Update() {
	...
	//Make it rerender each time not good but eh
